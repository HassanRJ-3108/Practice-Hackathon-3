...
import { bestsellerProductsQuery } from '@/sanity/lib/queries';
...
export default function BestSelling() {
    ...
    useEffect(() => {
        async function fetchProducts() {
            const products: Product[] = await client.fetch(bestsellerProductsQuery);
            setProducts(products.slice(0, 4)); // Get only top 4 products
        }
                            ...
                            <Link href={`/product/${product.slug.current}`} legacyBehavior>
                                <a>
                                    <div className="relative aspect-square bg-[#F5F5F5] rounded-[4px] p-3 sm:p-4">
                                        ...
                                        {product.image && (
                                            <div className="w-full h-full bg-white flex items-center justify-center overflow-hidden rounded-[4px]">
                                                <Image
